...
class MidiFileConverter(converters_frontends_abc.FileConverter):
    ...
    def __init__(
        ...
        if n_midi_channels_per_track is None:
            n_midi_channels_per_track = midi.constants.DEFAULT_N_MIDI_CHANNELS_PER_TRACK
        ...
        if tempo_events is None:
            tempo_events = midi.constants.DEFAULT_TEMPO_EVENTS
    ...
    def _assert_available_midi_channels_have_correct_value(
        ...
        for midi_channel in available_midi_channels:
            try:
                assert midi_channel in midi.constants.ALLOWED_MIDI_CHANNELS
            except AssertionError:
                message = (
                    "Found unknown midi channel '{}' in available_midi_channels."
                    .format(midi.constants.ALLOWED_MIDI_CHANNELS)
                )
                message += " Only midi channel '{}' are allowed.".format(
                    midi.constants.ALLOWED_MIDI_CHANNELS
                )
                raise ValueError(message)
    ...
    def _volume_to_velocity(volume: numbers.Number) -> int:
        ...
        """
        velocity = int(round(volume * midi.constants.MAXIMUM_VELOCITY))

        # clip velocity
        if velocity < midi.constants.MINIMUM_VELOCITY:
            velocity = midi.constants.MINIMUM_VELOCITY
        if velocity > midi.constants.MAXIMUM_VELOCITY:
            velocity = midi.constants.MAXIMUM_VELOCITY
    ...
    def _cent_deviation_to_pitch_bending_number(
        ...
        if abs(cent_deviation) > 0.15:
            ...
            pitch_bending_number = int(
                midi.constants.MAXIMUM_PITCH_BEND * pitch_bend_percent
            )
            pitch_bending_number -= midi.constants.NEUTRAL_PITCH_BEND
    ...
    def _tune_pitch(
        ...
        difference_in_cents_to_closest_midi_pitch = parameters.pitches.abc.Pitch.hertz_to_cents(
    ...
    def _tempo_events_to_midi_messages(
        ...
        for absolute_time, tempo_event in zip(
            ...
            if beat_length_in_seconds >= midi.constants.MAXIMUM_MICROSECONDS_PER_BEAT:
                beat_length_in_seconds = midi.constants.MAXIMUM_MICROSECONDS_PER_BEAT
                message = (
    ...
    def _extracted_data_to_midi_messages(
        ...
        available_midi_channels_cycle: typing.Iterator,
        pitch_or_pitches: typing.Tuple[parameters.pitches.abc.Pitch],
